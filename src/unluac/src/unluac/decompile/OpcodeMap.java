  ...
  public OpcodeMap(Version.OpcodeMapType type) {
    ...
    switch(type) {
      ...
      case LUA51:
        ...
        map[0] = null;
        map[1] = Op.MOVE;       // LOADK
        map[2] = Op.LOADK;      // LOADBOOL
        map[3] = Op.LOADBOOL;   // LOADNIL
        map[4] = Op.LOADNIL;    // GETUPVAL
        map[5] = Op.GETUPVAL;   // GETGLOBAL
        map[6] = Op.GETGLOBAL;  // GETTABLE
        map[7] = Op.GETTABLE;   // SETGLOBAL
        map[8] = Op.SETUPVAL;   // (skip)
        map[9] = Op.SETGLOBAL;  // SETTABLE
        map[10] = Op.SETUPVAL;  // NEWTABLE
        map[11] = Op.SETTABLE;  // SELF
        map[12] = Op.NEWTABLE;  // ADD
        map[13] = Op.SUB;       // (skip)
        map[14] = Op.SELF;      // MUL
        map[15] = Op.DIV;       // (skip)
        map[16] = Op.ADD;       // MOD
        ...
        map[19] = Op.DIV;       // NOT
        map[20] = Op.MOD;       // LEN
        map[21] = Op.POW;       // CONCAT
        map[22] = Op.UNM;       // JMP
        map[23] = Op.NOT;       // EQ
        map[24] = Op.LEN;       // LT
        map[25] = Op.CONCAT;    // LE
        map[26] = Op.TEST;      // (skip)
        map[27] = Op.JMP;       // TESTSET
        map[28] = Op.EQ;        // CALL
        map[29] = Op.LT;        // TAILCALL
        map[30] = Op.LE;        // RETURN
        map[31] = Op.TEST;      // FORLOOP
        map[32] = Op.TESTSET;   // FORPREP
        map[33] = Op.CALL;      // TFORLOOP
        map[34] = Op.TAILCALL;  // SETLIST
        map[35] = Op.CLOSE;     // (skip)
        map[36] = Op.RETURN;    // CLOSURE
        map[37] = Op.FORLOOP;   // VARARG
